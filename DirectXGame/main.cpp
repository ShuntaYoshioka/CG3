...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//descriptorのサイズを取得
	const uint32_t descriptorSizeRTV = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	const uint32_t descriptorSizeSRV = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
	...
	D3D12_ROOT_PARAMETER rootParmeters[5]{};
	...
	rootParmeters[4].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	rootParmeters[4].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
	rootParmeters[4].Descriptor.ShaderRegister = 2;
	...
	*/
	...
	vertexBufferView.SizeInBytes = UINT(sizeof(VertexData) * kNumSphereVertices); // 使用するリソースのサイズは頂点のサイズ
